...
export default function FinanzasApp() {
  ...
  const addCompromiso = (c) => {
    const id = "com_" + Date.now();
    const nextC = [...compromisos, { ...c, id, pagado: false }];
    setCompromisos(nextC);
    persist("compromisos", nextC);
  };

  const deleteCompromiso = (id) => {
    const nextC = compromisos.filter((c) => c.id !== id);
    setCompromisos(nextC);
    persist("compromisos", nextC);
    // Si estaba pagado, eliminar también el movimiento vinculado
    const nextM = movements.filter((m) => m.fromCompromiso !== id);
    if (nextM.length !== movements.length) {
      setMovements(nextM);
      persist("movements", nextM);
    }
  };

  const pagarCompromiso = (id) => {
    const c = compromisos.find((x) => x.id === id);
    if (!c) return;
    // Marcar como pagado
    const nextC = compromisos.map((x) => x.id === id ? { ...x, pagado: true } : x);
    setCompromisos(nextC);
    persist("compromisos", nextC);
    // Crear movimiento real en Presupuesto
    const mov = {
      id: "mov_" + id,
      kind: "gasto",
      amount: c.amount,
  ...
  const [cicloOffset, setCicloOffset] = useState(0); // 0=actual, -1=anterior, +1=siguiente

  const domingoActivo = useMemo(() => {
    const base = getCicloActivo(domingoRef);
    if (cicloOffset === 0) return base;
    const d = new Date(base + "T00:00:00");
    d.setDate(d.getDate() + cicloOffset * 28);
    return toISO(d);
  }, [domingoRef, cicloOffset]);
        ...
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            ...
            <div style={{ display: "flex", gap: 4, alignItems: "center", marginTop: 4 }}>
              <button onClick={() => setCicloOffset(o => o - 1)} style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, padding: 8, color: "#F7F4EC", cursor: "pointer" }}><ChevronLeft size={16} /></button>
              {cicloOffset !== 0 && <button onClick={() => setCicloOffset(0)} style={{ background: "rgba(255,255,255,0.15)", border: "none", borderRadius: 8, padding: "6px 10px", color: "#F7F4EC", fontSize: 11, cursor: "pointer" }}>hoy</button>}
              <button onClick={() => setCicloOffset(o => o + 1)} style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, padding: 8, color: "#F7F4EC", cursor: "pointer" }}><ChevronRight size={16} /></button>
            </div>
          </div>
...
function ApartadosView({ accounts, movements, budgetCategories, incomeTemplate, asignaciones, compromisos, ciclo, semanasApartado, onAddIncome, onDeleteIncome, onAddAsignacion, onDeleteAsignacion, onAddCompromiso, onDeleteCompromiso, onPagarCompromiso }) {
  ...
  return (
    <div>
                    ...
                    return (
                      <div key={inc.id} style={{ padding: "10px 16px", borderTop: "1px solid #F0ECE0" }}>
                        ...
                        {compromisosWeek.filter(c => c.incomeTemplateId === inc.id).map((c) => (
                          <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0 6px 12px", fontSize: 12 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <button
                                onClick={() => !c.pagado && onPagarCompromiso(c.id)}
                                style={{
                                  width: 18, height: 18, borderRadius: 4, border: c.pagado ? "none" : "2px solid #6B8F71",
                                  background: c.pagado ? "#6B8F71" : "transparent", cursor: c.pagado ? "default" : "pointer",
                                  display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0
                                }}
                              >
                                {c.pagado && <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>✓</span>}
                              </button>
                              <div>
                                <span style={{ color: c.pagado ? "#A39E8F" : "#7A7568", textDecoration: c.pagado ? "line-through" : "none" }}>
                                  {c.label} <span style={{ fontSize: 10, color: "#A39E8F" }}>(débito)</span>
                                </span>
                                {c.pagado && <span style={{ fontSize: 10, color: "#6B8F71", marginLeft: 4 }}>· en Presupuesto ✓</span>}
                              </div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              <span style={{ fontWeight: 600, color: c.pagado ? "#A39E8F" : "#1C2541" }}>{fmt(c.amount)}</span>
                              {!c.pagado && <button onClick={() => onDeleteCompromiso(c.id)} style={{ background: "none", border: "none", color: "#C9BFA8" }}><X size={12} /></button>}
                            </div>
                          </div>
                        ))}
      ...
      {addingCompromisoFor && <CompromisoModal info={addingCompromisoFor} accounts={accounts.filter(a => a.type === "Débito")} budgetCategories={budgetCategories} ciclo={ciclo} onClose={() => setAddingCompromisoFor(null)} onSave={(data) => { onAddCompromiso({ pagoISO: ciclo.pago, weekIdx: addingCompromisoFor.weekIdx, incomeTemplateId: addingCompromisoFor.incomeId, ...data }); setAddingCompromisoFor(null); }} />}
    </div>
  );
}
...
function CompromisoModal({ info, accounts, budgetCategories, ciclo, onClose, onSave }) {
  ...
  const submit = () => {
    if (!amount || !accountId || !categoryId || !subcategoryId) return;
    onSave({
      label: label || subOptions.find(s => s.id === subcategoryId)?.name || "",
      amount: Math.min(Number(amount), info.remaining),
      accountId,
      categoryId,
      subcategoryId,
      date,
  ...
  return (
    <ModalShell title="Gasto proyectado en débito" onClose={onClose}>
      <div style={{ background: "#F0F7F0", border: "1px solid #C3E0C3", borderRadius: 10, padding: "8px 12px", fontSize: 12, color: "#4A7A4A", marginBottom: 14 }}>
        Se registrará como gasto real en Presupuesto automáticamente.
        Disponible: <strong>{fmt(info.remaining)}</strong>
      </div>
      ...
      <select style={iS} value={accountId} onChange={(e) => setAccountId(e.target.value)}>
        {accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
      </select>

      <label style={lS}>Categoría</label>
      <select style={iS} value={categoryId} onChange={(e) => handleCategoryChange(e.target.value)}>
        {budgetCategories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
      </select>

      <label style={lS}>Subcategoría</label>
      <select style={iS} value={subcategoryId} onChange={(e) => setSubcategoryId(e.target.value)}>
        {subOptions.length === 0 && <option value="">Sin subcategorías</option>}
        {subOptions.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
      </select>

      <label style={lS}>Descripción (opcional)</label>
      <input style={iS} type="text" placeholder="Ej. Walmart, gasolina Pemex…" value={label} onChange={(e) => setLabel(e.target.value)} />

      <label style={lS}>Fecha · ciclo {new Date(ciclo.inicio+"T00:00:00").toLocaleDateString("es-MX",{day:"numeric",month:"short"})} – {new Date(ciclo.finGasto+"T00:00:00").toLocaleDateString("es-MX",{day:"numeric",month:"short"})}</label>
      <input style={iS} type="date" value={date} onChange={(e) => setDate(e.target.value)} />
      {fueraDelCiclo && <div style={{ background: "#FEF3CD", border: "1px solid #F0D080", borderRadius: 10, padding: "8px 12px", fontSize: 12, color: "#7A6020", marginBottom: 12 }}>⚠️ Fecha fuera del ciclo activo.</div>}

      <button onClick={submit} style={{ width: "100%", padding: 14, background: "#6B8F71", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 600 }}>Agregar gasto</button>
    </ModalShell>
  );
}
